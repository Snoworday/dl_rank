...
def distributeTrain(conf, date, ps_num, ev_num, emrName, keyFile, logpath=emr_home_path, project_path=''):
    ...
    def run_remote(wi, TF_CONFIG, UserName, hostname, is_eval):
        ...
        if __name__=='__main__':
            ...
            trainpath = '-m dl_rank.local'
        cmd = r'''
        python3 {trainpath} --tw {tw} --wi {wi} --useps {useps} {date} --conf {conf} --tfconfig {tfconfig} --logpath {logpath}
        '''.format(trainpath=trainpath, tw=total_worker if not is_eval else 1, wi=wi, useps=ps_num != 0,
    ...
    def run_local(wi, TF_CONFIG):
        ...
        cmd = '''
        python3 {trainpath} --tw {tw} --wi {wi} --useps {useps} {date} --conf {conf} --tfconfig {tfconfig} --logpath {logpath}
        '''.format(trainpath=trainpath, tw=total_worker, wi=wi, useps=ps_num != 0, date=date, conf=conf, tfconfig=TF_CONFIG, logpath=os.path.join(logpath, 'dl_rank_train.log'))
        nohup_cmd = '/usr/bin/nohup' + cmd + ' > {} 2>&1 &'.format(os.path.join(logpath, 'dl_rank_run.log'))
        subprocess.Popen([nohup_cmd], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)
        # _ = os.system(cmd)
...
def stopCluster(emrName, keyFile):
    ...
    for node in hostnameList:
        if node == chief:
            os.system("ps -ef | grep python | grep local | awk '{print \"kill -9 \" $2}' | bash -v")
        else:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(hostname=node, port=port, username=UserName, key_filename=keyFile)
            _ = ssh.exec_command("ps -ef | grep python | grep local | awk '{print \"kill -9 \" $2}' | bash -v")
            ssh.close()
...
def run_tensorboard(conf='', model_dir='', logpath=emr_home_path, port=6006):
    ...
    subprocess.Popen([nohup_cmd], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
