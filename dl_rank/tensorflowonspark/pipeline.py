...
when inferencing on the executors.  The executors will run in parallel, but the TensorFlow models must fit in the memory
of each executor.
...
class HasModelDir(Params):
  model_dir = Param(Params._dummy(), "model_dir", "Path to save/load models checkpoints", typeConverter=TypeConverters.toString)
...
class Namespace(object):
  ...
  def __init__(self, d):
    if isinstance(d, list):
      ...
      raise Exception("Unsupported Namespace _args: {}".format(d))
...
class TFParams(Params):
  """Mix-in class to store namespace-style _args and merge w/ SparkML-style params."""
  args = None

  def merge_args_params(self):
    local_args = copy.copy(self.args)                 # make a local copy of _args
...
class TFEstimator(Estimator, TFParams, HasInputMapping,
  ...
  def _fit(self, dataset):
    """Trains a TensorFlow models and returns a TFModel instance with the same _args/params pointing to a checkpoint or saved_model on disk.

    Args:
      :dataset: A Spark DataFrame with columns that will be mapped to TensorFlow tensors.

    Returns:
      A TFModel representing the trained models, backed on disk by a TensorFlow checkpoint or saved_model.
    """
    sc = SparkContext.getOrCreate()

    logging.info("===== 1. train _args: {0}".format(self.args))
    logging.info("===== 2. train params: {0}".format(self._paramMap))
    local_args = self.merge_args_params()
    logging.info("===== 3. train _args + params: {0}".format(local_args))
...
class TFModel(Model, TFParams,
  ...
  """Spark ML Model backed by a TensorFlow models checkpoint/saved_model on disk.

  During ``transform()``, each executor will run an independent, single-node instance of TensorFlow in parallel, so the models must fit in memory.
  The models/session will be loaded/initialized just once for each Spark Python worker, and the session will be cached for
  subsequent tasks/partitions to avoid re-loading the models for each partition.
  ...
  def _transform(self, dataset):
    ...
    # merge _args + params
    logging.info("===== 1. inference _args: {0}".format(self.args))
    logging.info("===== 2. inference params: {0}".format(self._paramMap))
    local_args = self.merge_args_params()
    logging.info("===== 3. inference _args + params: {0}".format(local_args))
...
global_args = None            # _args provided to the _run_model() method.  Any change will invalidate the global_sess cache.


def _run_model(iterator, args, tf_args):
  """mapPartitions function to run single-node inferencing from a checkpoint/saved_model, using the models's input/output mappings.

  Args:
    :iterator: input RDD partition iterator.
    :_args: arguments for TFModel, in argparse format
  ...
  if global_sess and global_args == args:
    # if graph/session already loaded/started (and using same _args), just reuse it
    sess = global_sess
  else:
    ...
    if args.export_dir:
      ...
      assert ckpt, "Invalid models checkpoint path: {}".format(args.model_dir)
...
def single_node_env(args):
  ...
  Args:
    :_args: command line arguments as either argparse _args or argv list
  """
