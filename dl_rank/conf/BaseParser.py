...
class BaseParser(object):
    ...
    def __init__(self, confName, useSpark):
    ...
    def load_conf_switch(self):
        if self.useSpark:
            ...
            def wrapper(filename):
                try:
                    with open(os.path.join(os.path.dirname(__file__), self.confName, filename), 'r') as f:
                        return yaml.load(f)
                except:
                    return None
            return wrapper
    ...
    def serving_parse_fn(self):
        ...
        def serving_input_receiver_fn():
            ...
            for key, value in csv_defaults.items():
                with tf.variable_scope(csv_scope[key][0]) as scope:
    ...
    def parse_fn(self, isPred=False, na_value='', get_serving_input_receiver_fn=False, tail=''):
        ...
        def parser(value):
            ...
            if isPred:
                features_tail.update({elem+tail: data for elem, data in data_container.items() if elem != 'features'})
