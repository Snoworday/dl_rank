...
from pyspark.sql.functions import sum, desc, collect_list, rand
...
itemid = new_itemid_row.union(itemid).rdd.zipWithIndex().map(lambda x: (x[0][0], x[1]))
pid_emb = itemid.toDF(['key', 'value'])
## cid_path
cid = cid # spark.read.text(cid_path)
new_cid_row = spark.createDataFrame([['DEFAULT']])
cid = new_cid_row.union(cid).rdd.zipWithIndex().map(lambda x: (x[0][0], x[1]))
cid_emb = cid.toDF(['key', 'value'])
## itemid_cross_path
itemid_cross = cross # spark.read.text(itemid_cross_path)
new_itemid_cross_row = spark.createDataFrame([['DEFAULT']])
itemid_cross = new_itemid_cross_row.union(itemid_cross).rdd.zipWithIndex().map(lambda x: (x[0][0], x[1]))
cross_emb = itemid_cross.toDF(['key', 'value'])
## item_profile_path
