...
if DEBUG:
    ...
    if train:
        labelPath = 's3://jiayun.spark.data/wangqi/I2IRank/I2ICTRUV/label_ctr_uv/{}'.format(_args.date)
        trainDFOutPath = "s3://jiayun.spark.data/wangqi/I2IRank/I2ICTRUV/traindata/{}".format(_args.date)
        evalDFOutPath = "s3://jiayun.spark.data/wangqi/I2IRank/I2ICTRUV/evaldata/{}".format(_args.date)
    else:
        ...
        # labelPath = 's3://jiayun.spark.data/wangqi//I2IRankData/TextFile/LabelData/'
        predDFOutPath = 's3://jiayun.spark.data/wangqi/I2IRank/I2ICTRUV/preddata/{}'.format(_args.date)
...
#read profile
itemProfileDF = spark.read.text(itemProfilePath).rdd.repartition(10000).filter(lambda x:x[0].split('|')[1] != 'pid').toDF()
itemProfileDF = itemProfileDF.withColumn('pid', colFactory(itemProfileDF.value))
#read label
if train:
    rawLabelDF = spark.read.text(labelPath).rdd.repartition(50000).map(lambda input_: input_.value.split('|')).filter(
    ...
    rawLabelDF = spark.read.text(labelPath).rdd.repartition(20000).map(
...
def _toOut(row):
    row = row[0]
    elem1, elem2, f1, f2 = row.split(primary_delim)
    f1 = f1.split('|')
    f2 = f2.split('|')
    merge = list(zip(f1, f2))
    merge_t = [teriary_delim.join(attr) for attr in merge]
    merge_s = secondary_delim.join(merge_t)
    out = primary_delim.join([elem1, elem2, merge_s])
    out = (out, )
    return out
...
if train:
    ...
    DF3_sample = DF2_sample.withColumn('merge', concat_ws(primary_delim, DF2_sample['ctr_label'], DF2_sample['uv_label'], DF2_sample['features1'], DF2_sample['features2']))
    DF3_sample.select('merge').rdd.map(_toOut).toDF().write.text(trainDFOutPath)
    ...
    DF3 = DF2.withColumn('merge', concat_ws(primary_delim, DF2['ctr_label'], DF2['uv_label'], DF2['features1'], DF2['features2']))
    DF3.select('merge').rdd.map(_toOut).toDF().write.text(evalDFOutPath)
    ...
    DF2 = union_label_feature(LabelDF, itemProfileDF)
    # DF2 = DF2.withColumn('features1_features2', u_mergeTwoItemProfile(DF2.features1, DF2.features2)).drop('features1').drop('features2')
    DF3 = DF2.withColumn('merge', concat_ws(primary_delim, DF2['item1'], DF2['item2'], DF2['features1'], DF2['features2']))
    DF3 = DF3.select('merge').rdd.map(_toOut).toDF().write.text(predDFOutPath)
