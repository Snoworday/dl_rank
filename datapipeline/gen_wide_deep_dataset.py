...
def GenRecord(row):
    ...
    time = [datetime.strptime(t.split('.')[0], "%Y-%m-%d %H:%M:%S") for t in imptime + clicktime + ordertime]
...
pid_emb = itemid_append.withColumnRenamed('_1', 'key').withColumn('value', row_number().over(Window.orderBy(monotonically_increasing_id())) - 1)

## cid_path
cid = cid # spark.read.text(cid_path)
new_cid_row = spark.createDataFrame([['DEFAULT']])
cid_append = new_cid_row.union(cid)
cid_emb = cid_append.withColumnRenamed('_1', 'key').withColumn('value', row_number().over(Window.orderBy(monotonically_increasing_id())) - 1)
...
item_profile = item_profile.map(
    lambda x: tuple([x.split('|')[1]] + [float(x.split('|')[col]) if x.split('|')[col] != '' else -math.inf for col in
                                         item_col])).persist()
if min_max_tag:
    ...
    min_max_group = []
    for b_idx in range(1, len(item_col)+1):
        b1 = item_profile.map(lambda x: x[b_idx]).filter(lambda x: x!=-math.inf).sortBy(lambda x: x).persist()
        b1_count = b1.count()
        min_num, max_num = b1_count//group_num, b1_count - b1_count//group_num
        min_max_array = b1.zipWithIndex().filter(lambda x: x[1] in [min_num, max_num]).take(2)

        min_max_group.append((min_max_array[0][0], min_max_array[1][0]))
    item_profile_normal = item_profile.map(lambda x: tuple(
        [x[0]] + [0 if i == -math.inf else 1
        if i<=min_max[0] else group_num-1
        if i>=min_max[1] else math.ceil((group_num - 3) * (i - min_max[0]) / (min_max[1]-min_max[0]))
                  for i, min_max in zip(x[1:], min_max_group)]))
...
# save statistic data
cid_count = cid_append.count()
itemid_count = itemid_append.count()
itemid_cross_count = itemid_cross_append.count()
id_map = {'cid': cid_count, 'itemid': itemid_count, 'itemid_cross': itemid_cross_count}
feature_map = {name: (*min_max, 3000) for name, min_max in zip(item_name, min_max_group)}
feature_map.update(id_map)
with open(profile_tmp_path, 'w') as f:
